...
import { getDoctorsByDepartmentID } from "../../services/doctorService";
import {
  addPrescription,
...
const AppointmentDetailView = () => {
  ...
  const fetchAppointments = async (page) => {
    try {
      ...
      // Fetch appointments depending on the current user's role
      if (currentUser?.role === "admin") {
        response = await getAppointments(page, appointmentsPerPage);
      } else if (currentUser?.role === "doctor") {
        response = await getAppointmentsByDoctorID(doctorID);
      } else if (currentUser?.role === "patient") {
        response = await getAppointmentsByPatientID(patientID);
      } else {
      ...
      if (response?.success === false) {
        setErrorMessage(
          response?.message || "Failed to fetch appointments data."
        );
        toast.error(response?.message || "Failed to fetch appointments data.");
        setLoading(false);
        return;
      }

      // Handle appointment data based on user role
      if (currentUser?.role === "doctor") {
        setAppointmentsDoctors(response.appointments || []);
      } else if (currentUser?.role === "patient") {
        setAppointmentsPatients(response.appointment || []);
      } else {
  ...
  const fetchAppointmentsByID = async (appointmentID) => {
    try {
      ...
      setAppointmentStatus(response.appointment_status || "N/A");
      setAppointmentType(response.appointment_type || "N/A");
      setAppointmentTime(response.appointment_time || "N/A");
      setAppointmentDate(response.appointment_date || "N/A");
      setDoctorId(response.doctor_id || "N/A");
      setDepartmentId(response.department_id || "N/A");
      setPatientId(response.patient_id || "N/A");
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      setErrorMessage(null);
      const response = await deleteAppointment(appointmentIdToDelete);
  ...
  const fetchPrescriptionsByAppointmentID = async (appointmentID) => {
    try {
      ...
      setPrescriptionDate(response.prescription_date);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateAppointment(appointmentId, updatedData);
      await fetchAppointments(1);
  ...
  const handlePrescriptionSubmit = async (e) => {
    ...
    try {
      ...
      if (prescriptionId) {
        response = await updatePrescription(prescriptionId, updatedData);
      } else {
        response = await addPrescription(updatedData);
      }

      await fetchAppointments(1);
      toast.success(prescriptionId ? response.message : response.message);
  ...
  return (
    <NavbarSidebar isFooter={false}>
                    ...
                    appointments.map((appointment) => (
                      ...
                      >
                        <TableRow className="hover:bg-gray-100 text-center">
                          ...
                          <TableCell className="max-w-96 flex justify-center items-center gap-x-4 whitespace-nowrap">
                            <Button
                              color={
                                appointment.appointment_status ===
                                "Pending with admin"
                ...
                <Table className="min-w-full divide-y divide-gray-200">
                  <TableHead className="bg-gray-100 text-center">
                    <TableRow>
                      ...
                      <TableHeadCell>Appointment Type</TableHeadCell>
                      <TableHeadCell>Appointment Status</TableHeadCell>
                      <TableHeadCell>Prescribe</TableHeadCell>
                      <TableHeadCell>Actions</TableHeadCell>
                    </TableRow>
                  </TableHead>
                  {errorMessage ? (
      ...
      <Modal onClose={() => setOpen(false)} show={isOpen} size="md">
        <ModalHeader className="px-6 pb-0 pt-6 text-2xl font-bold mb-4 text-center uppercase">
              ...
              <Table className="w-full">
                <TableBody>
                  <TableRow className="border-b border-gray-300">
                    ...
                    <TableCell className="py-2 whitespace-pre-wrap">
